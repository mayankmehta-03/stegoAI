...
CHECKPOINT="checkpoints/steg_model-{epoch:02d}-{val_loss:.2f}.keras"
PRETRAINED='checkpoints/steg_model-04-0.03.hdf5'  # Load from old format if available

# Configure batch size - Increased for better quality
BATCH_SIZE=64
...
def generate_generator_multiple(generator, direc):
    ...
    while True:
            X1i = normalize_batch(next(genX1)) 
            X2i = normalize_batch(next(genX2))
...
def custom_loss_1(secret,secret_pred):
    ...
    return keras.losses.MeanSquaredError()(secret, secret_pred)

def custom_loss_2(cover,cover_pred):
    # Compute L2 loss(MSE) for cover image
    return keras.losses.MeanSquaredError()(cover, cover_pred) 
...
# Loss weights - Balanced for better color preservation
lossWeights = {"hide_conv_f": 1.0, "revl_conv_f": 1.0}    
...
def steg_model(pretrain=False):
    ...
    # Noise layer - Reduced noise for better quality
    noise_ip = GaussianNoise(0.02)(cover_pred)
...
# Plot graph (requires graphviz - skipping for now)
...
# TF file writer for images (deprecated in TF 2.x - skipping)
...
# Train the model - 150 epochs for higher quality
model.fit(inputgenerator, epochs=150, 
